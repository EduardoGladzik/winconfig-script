...
function Set-AutoUpdates {
    Write-Host "Configurando Windows Update (Oficial + Recursos)..."
...
function Install-ChocoPrograms {
    ...
    if (-not $scriptPath) { $scriptPath = Get-Location } # Fallback
    $configFile = Join-Path -Path $scriptPath -ChildPath "packages.config"
    ...
    if (!(Test-Path "$env:ProgramData\chocolatey\bin\choco.exe")) {
        ...
        catch { throw "Falha ao baixar Chocolatey. Sem internet?" }
    }
    ...
    # UNINSTALL CHOCOLATEY (BROKEN).
    #Write-Host "  > Removendo o Chocolatey do sistema (Limpeza)..."
    #$chocoPath = "$env:ProgramData\chocolatey"
    #if (Test-Path $chocoPath) {
    #    Remove-Item $chocoPath -Recurse -Force -ErrorAction SilentlyContinue
    #    Write-Host "  > Chocolatey removido."
    #}
}

function Set-SleepTime {
    Write-Host "Ajustando energia (1 hora)..."
...
function Set-ComputerName($novoNome) {
    Write-Host "AVISO: Alterando nome do PC para $novoNome."
...
$janela.Text = "Ferramenta de Configuração (Completa)"
$janela.Size = New-Object System.Drawing.Size(500, 550) # Aumentei um pouco a altura
...
$janela.Controls.Add($check_InstallApps)


$botaoExecutar = New-Object System.Windows.Forms.Button
$botaoExecutar.Text = "APLICAR CONFIGURAÇÕES"
$botaoExecutar.Location = New-Object System.Drawing.Point(100, 420)
